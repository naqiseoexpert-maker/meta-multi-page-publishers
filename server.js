...
export default {
  async fetch(request, env) {
    try {
      ...
      if (request.method === "GET" && path === "/") return showDashboard(env);
      if (request.method === "GET" && path === "/auth/meta") return startMetaLogin(request, env);
      if (request.method === "GET" && path === "/auth/meta/callback") return metaCallback(request, env);
      if (request.method === "POST" && path === "/sync") return syncPages(request, env);
      if (request.method === "POST" && path === "/remove-account") return removeAccount(request, env);
      if (request.method === "POST" && path === "/publish") return publishPost(request, env);
      if (request.method === "GET" && path === "/logout") return handleLogout();
      ...
      return page(
        "Error",
        '<div class="error-box"><h2>Something went wrong</h2><pre>' +
          escapeHtml(
            error && (error.stack || error.message)
              ? error.stack || error.message
              : String(error)
          ) +
          '</pre><a class="back-btn" href="/">Back to Dashboard</a></div>'
      );
    }
  }
};
...
async function showDashboard(env) {
  ...
  return page(
    ...
    '<div class="hero-brand">' +
      '<div class="hero-kicker">META PUBLISHING COMMAND CENTER</div>' +
      '<div class="hero-name">NAQI SHAH</div>' +
      '<div class="hero-title">Meta Multi Page Publisher</div>' +
      '<div class="hero-subtitle">Manage accounts, select Pages and publish content from one professional dashboard.</div>' +
    '</div>' +
    ...
    "<script>" +
      ...
      "function updateSelectedCount() {" +
        ...
        "if (counter) {" +
          "counter.textContent = checked.length + ' Page' + " +
      ...
      "function selectAccountPages(accountId, select) {" +
        ...
        "document.querySelectorAll('.account-' + accountId).forEach(function(checkbox) {" +
      ...
      "(function(){" +
        ...
        "if (params.get('fresh') === '1') {" +
          ...
          "sessionStorage.setItem('mp_dashboard_session', '1');" +
          "window.history.replaceState({}, document.title, '/');" +

        "} else if (nav && nav.type === 'reload') {" +

          "sessionStorage.removeItem('mp_dashboard_session');" +
          "window.location.replace('/logout');" +
          "return;" +

        "} else if (!sessionStorage.getItem('mp_dashboard_session')) {" +
...
async function publishPost(
   ...
   * Text:
   *   3 Pages per batch
   *   3.5 seconds between batches
   *
   * Image / Video:
   *   1 Page per batch
   *   5 seconds between uploads
   *
   * This prevents 100+ Graph API requests from being fired
   * at the same time.
   */
  ...
  for (
    ...
    /*
     * Meta Code 368 mila to foran stop.
     * Is error ko dobara retry nahi karna.
     */
    if (
      securityRestrictionDetected
    ) {
  ...
  /*
   * Agar Code 368 aya hai to jo Pages abhi start nahi hue
   * unko request bhejne ke bajaye skipped mark kar dein.
   */
  ...
  return new Response(
    ...
    "<head>" +
      ...
      "<style>" +
        ...
        ".hero-brand{" +
          "background:linear-gradient(135deg,#0b1730,#123b75 58%,#1877f2);" +
          "color:#fff;" +
